...
export default function HomepageManage() {
  ...
  const [heroPrimaryButton, setHeroPrimaryButton] = useState({ text: '', link: '' });
  const [heroSecondaryButton, setHeroSecondaryButton] = useState({ text: '', link: '' });
  ...
  const fetchHomepageContent = async () => {
    try {
      ...
      if (data.hero) {
        ...
        setHeroPrimaryButton(data.hero.primaryButtonText && data.hero.primaryButtonLink 
          ? { text: data.hero.primaryButtonText, link: data.hero.primaryButtonLink }
          : { text: '', link: '' });
        setHeroSecondaryButton(data.hero.secondaryButtonText && data.hero.secondaryButtonLink
          ? { text: data.hero.secondaryButtonText, link: data.hero.secondaryButtonLink }
          : { text: '', link: '' });
      }

      // Set services (use default if empty)
      setServices(data.services && data.services.length > 0 
        ? data.services 
        : [
      ...
      if (data.whyChooseUs) {
        ...
        setWhyStats(data.whyChooseUs.stats && data.whyChooseUs.stats.length > 0
      ...
      if (data.cta) {
        ...
        setCtaButton(data.cta.buttonText && data.cta.buttonLink
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.success('Homepage content saved successfully!', { id: loadingToast });
    } catch (err) {
      toast.error(err.response?.data?.error || 'Failed to save homepage content', { id: loadingToast });
      console.error(err);
    } finally {
      setSaving(false);
    }
      ...
      <AdminLayout currentPage="homepage">
        <div className="mb-6 flex justify-between items-center">
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-lg shadow p-6">
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block mb-2 font-medium">Primary Button Text</label>
                    <input
                      type="text"
                      value={heroPrimaryButton.text}
                      onChange={(e) => setHeroPrimaryButton({ ...heroPrimaryButton, text: e.target.value })}
                      className="w-full px-4 py-2 border rounded-lg"
                      placeholder="Hire Us"
                    />
                  </div>
                  <div>
                    <label className="block mb-2 font-medium">Primary Button Link</label>
                    <input
                      type="text"
                      value={heroPrimaryButton.link}
                      onChange={(e) => setHeroPrimaryButton({ ...heroPrimaryButton, link: e.target.value })}
                      className="w-full px-4 py-2 border rounded-lg"
                      placeholder="/contact"
                    />
                  </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block mb-2 font-medium">Secondary Button Text</label>
                    <input
                      type="text"
                      value={heroSecondaryButton.text}
                      onChange={(e) => setHeroSecondaryButton({ ...heroSecondaryButton, text: e.target.value })}
                      className="w-full px-4 py-2 border rounded-lg"
                      placeholder="View Services"
                    />
                  </div>
                  <div>
                    <label className="block mb-2 font-medium">Secondary Button Link</label>
                    <input
                      type="text"
                      value={heroSecondaryButton.link}
                      onChange={(e) => setHeroSecondaryButton({ ...heroSecondaryButton, link: e.target.value })}
                      className="w-full px-4 py-2 border rounded-lg"
                  ...
                  <div key={index} className="border rounded-lg p-4">
                    ...
                    <div className="space-y-3">
                      ...
                      <div>
                        ...
                        />
                        <p className="mt-1 text-xs text-gray-500">Leave empty if no price should be shown</p>
                      </div>
                    </div>
                  </div>
                ))}
            ...
            {/* CTA Section */}
            <div className="bg-white rounded-lg shadow p-6">
              <h2 className="text-2xl font-semibold mb-4">Call-to-Action Section</h2>
              <div className="space-y-4">
                <div>
                  <label className="block mb-2 font-medium">Title</label>
                  <input
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setCtaButton({ ...ctaButton, text: e.target.value })}
                      className="w-full px-4 py-2 border rounded-lg"
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setCtaButton({ ...ctaButton, link: e.target.value })}
                      className="w-full px-4 py-2 border rounded-lg"
                      placeholder="/contact"
                    />
                  </div>
