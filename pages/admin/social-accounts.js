...
export default function SocialAccounts() {
  ...
  const [disconnectModal, setDisconnectModal] = useState({ isOpen: false, account: null });
  ...
  const handleDisconnect = async () => {
    ...
    try {
      ...
      toast.success(`${platform} account disconnected successfully`, { id: loadingToast });
      setDisconnectModal({ isOpen: false, account: null });
    } catch (err) {
      toast.error(err.response?.data?.error || 'Failed to disconnect account', { id: loadingToast });
    }
  };
    ...
    <ProtectedRoute>
      <div className="min-h-screen bg-gray-50">
          ...
          <div className="px-4 py-6 sm:px-0">
            <div className="mb-6">
              ...
              <p className="mt-2 text-gray-600">
                Connect your social media accounts to automatically post scheduled content.
              </p>
            </div>
            ...
            {loading ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {PLATFORMS.map((platform) => {
                  ...
                  const expired = connectedAccount ? isExpired(connectedAccount.expiresAt) : false;
                  const isAvailable = ['linkedin', 'facebook', 'instagram', 'twitter'].includes(platform.value);
                      ...
                      <div className="p-6">
                        <div className="flex items-center justify-between mb-4">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <h3 className="text-lg font-semibold text-gray-900">{platform.label}</h3>
                              {accountInfo?.username && (
                                <p className="text-sm text-gray-500">@{accountInfo.username}</p>
                              )}
                          ...
                          <div className="mb-4 p-3 bg-gray-50 rounded-lg">
                            ...
                            {expired && (
                              <div className="mt-2 text-xs text-red-600">
                                Token expired on {formatDate(connectedAccount.expiresAt)}
                              </div>
                            )}
                            {!expired && (
                              <div className="mt-2 text-xs text-gray-500">
                                Expires on {formatDate(connectedAccount.expiresAt)}
                              </div>
                            )}
                          </div>
                        )}
        ...
        <ConfirmModal
          ...
          message={disconnectModal.account ? `Are you sure you want to disconnect your ${disconnectModal.account.platform} account? You'll need to reconnect it to post to this platform.` : ''}
